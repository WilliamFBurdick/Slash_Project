...
void ASlashCharacter::Move(const FInputActionValue& Value)
{
	...
	if (CurrentValue.SquaredLength() > 0)
	{
		const FRotator YawRotation = FRotator(0.f, GetControlRotation().Yaw, 0.f);
		AddMovementInput(FRotationMatrix(YawRotation).GetUnitAxis(EAxis::X) * CurrentValue.Y);
		AddMovementInput(FRotationMatrix(YawRotation).GetUnitAxis(EAxis::Y) * CurrentValue.X);
	}
}
...
void ASlashCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		EnhancedInputComponent->BindAction(MoveAction, ETriggerEvent::Ongoing, this, &ASlashCharacter::Move);
		EnhancedInputComponent->BindAction(LookAction, ETriggerEvent::Triggered, this, &ASlashCharacter::Look);
		EnhancedInputComponent->BindAction(LookAction, ETriggerEvent::Ongoing, this, &ASlashCharacter::Look);
